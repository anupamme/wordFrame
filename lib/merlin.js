...
//////////////////////////////////////////////////////////////////////////////
  ...
  var getCellAddress = function (str) {

    var getColumnNumber = function (str) {
	...
	var getColumnCode = function (col) {
	...
	initialize: function ($el) {
	  ...
	  parser.lexer.cellHandler = this.parserHandler;

	  var i, j, a;
	  for (i=0; i<properties.rows; i+=1) {
	    a = [];
	    for (j=0; j<properties.cols; j+=1) {
	      a[j] = this.makeCell(i, j, this.initVal);
	    }
	    this.data[i] = a;
	  }
	},
	...
	trigger: function (e, params) {
	  if (this.eventsMap[e]) {
	    this.eventsMap[e].forEach(function (handler, index, array) {
	...
	on: function (e, handler) {
	  if (this.eventsMap[e]) {
	    this.eventsMap[e].push(handler);
	  } else {
	    this.eventsMap[e] = [handler];
	  }
	},

	isExpr: function (str) { return str.charAt(0)==='=' && str.length>1; },
	...
	getType: function (input) {
	  ...
	  else if (this.isExpr(input)) { return 'expr'; }
	  else { return 'string'; }
	},
	...
	evaluate: function (row, col, x) {
	  ...
	  return this.getType(x)==='expr'? p.parse(x.slice(1)).toString() : x;
	},

	getValues: function () {

	  var i, j, a, m = this.data.length, n = this.data[0].length, v = [];	  
	  for (i=0; i<m; i+=1) {
	    a = [];
	    for (j=0; j<n; j+=1) {
	      a[j] = this.data[i][j].value;
	    }
	    v[i] = a;
	  }
	  return v;
	...
	buildMenu: function () {
	  ...
	  this.$menu = $("<div></div>")
	    ...
	    .addClass(this.classNames.menu);
	  
	  this.menuButtons.forEach(function (btn, idx, array) {
	    this.$menu.append($("<a></a>")
			      .html(btn.html)
			      .attr('href', '#')
			      .attr('data-type', btn.name)
			      .attr('title', btn.helpText)
			      .addClass(this.classNames.button)
			      .on(btn.eventsMap, this));
	  }, this);
	  
	  this.$root.prepend(this.$menu);
	},
	...
	makeCell: function (row, col, input) {
	  ...
	  return {
	    ...
	    'value': this.evaluate(row, col, input),
	    'row': row,
	    'col': col,
	    dependencies: [],
	    dependents: []
	...
	update: function (changes) {
	  ...
	  for (k=0; k<n; k+=1) {
	    ...
	    this.updateCell(i, j, input);
	  }
	},

	updateCell: function (row, col, input) {

	  var cell = this.data[row][col];
	  if (input !== undefined) {
	    cell.type = this.getType(input);
	    cell.input = input;
	  }
	  cell.value = this.evaluate(row, col, cell.input);
	  if (cell.type === 'expr') {
	    this.$root.handsontable('setDataAtCell', row, col, cell.value, false, 'update');
	  }
	  this.trigger('cellUpdate', [row, col, input]);
	  this.updateDependents(cell);
	},

	updateDependents: function (cell) {

	  var i, d, len = cell.dependents.length;
	  for (i=0; i<len; i+=1) {
	    d = cell.dependents[i];
	    this.updateCell(d[0], d[1]);
	  }
	},

	beginEditMode: function (i, j) {
	...
	onBeginEditing: function (row, col, textarea, editProxy) {
	  ...
	  merlin.beginEditMode(row, col, textarea, editProxy);
	},

	onFinishEditing: function () {

	  merlin.finishEditMode();
	},
  ...
  $.fn.extend({
	...
	.handsontable('loadData', merlin.getValues());
    }
