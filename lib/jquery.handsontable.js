...
(function ($, window, Handsontable) {
  ...
  Handsontable.Core = function (rootElement, settings) {
	...
	//hack by Rahul to fire an event on edit
	if (priv.settings.onBeginEditing) {
	  priv.settings.onBeginEditing.apply(self.rootElement[0], 
					     [priv.selStart.row, priv.selStart.col, priv.editProxy, this]);
	  priv.settings.editMode = true;
	}//endhack
	...
	//minor hack by Rahul to fire an event on finish editing
	if (priv.settings.onFinishEditing) {
	  priv.settings.onFinishEditing.apply(self.rootElement[0], []);
	  priv.settings.editMode = false;
	}//endhack
